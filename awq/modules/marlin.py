...
def mul(A, B, C, s, workspace, thread_k=-1, thread_n=-1, sms=-1):
    ...
    """
    marlin_ext.mul(A, B, C, s, workspace, thread_k, thread_n, sms)
...
@torch.no_grad()
def dequantize_weight(qweight, qzeros, scales, group_size, bits):
    ...
    # overflow checks
    unpacked_qweight = torch.bitwise_and(iweight, (2**bits) - 1)
    unpacked_qzeros = torch.bitwise_and(izeros, (2**bits) - 1)
